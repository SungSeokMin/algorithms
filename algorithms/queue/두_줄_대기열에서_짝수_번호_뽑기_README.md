...
- **왜 이 자료구조인가?**: 대기열은 순서를 유지하며 앞에서 꺼내고 뒤에 다시 넣는 작업이 핵심이다. 이는 FIFO 큐의 전형적인 사용 패턴이다. 조건을 만족하지 않는 원소(홀수)를 뒤로 보내는 "재순환" 패턴도 큐로 자연스럽게 표현된다.
- **핵심 패턴**: 조건부 필터링 + 재순환 패턴. 앞에서 꺼낸 원소를 조건에 따라 결과에 추가하거나 큐 뒤로 돌려보낸다. 순환 큐(Circular Queue) 시뮬레이션과 유사하다.
- **풀기 전 체크리스트**:
  - 추적해야 할 상태: 현재 큐의 상태, 수집된 짝수 번호 목록
  - 입력: 두 배열과 정수 k, 출력: 짝수 번호 배열 (길이 k 이하)
  - 엣지케이스: 짝수가 k개 미만인 경우 (무한루프 방지 필요), 빈 큐
- **일반적인 접근 순서**:
  1. 두 큐를 하나로 합침
  2. 앞에서 원소를 꺼냄 (dequeue)
  3. 짝수이면 결과 배열에 추가, 홀수이면 큐 뒤로 재삽입 (enqueue)
  4. 결과 배열 길이가 k가 되거나 큐가 비면 종료
...
```js
function solutionByClaude(queue1, queue2, k) {
  const queue = [...queue1, ...queue2];
  const result = [];
  let head = 0;
