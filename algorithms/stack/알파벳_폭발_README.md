...
- **왜 이 자료구조인가?**: 폭발은 항상 "방금 처리한 문자"와 "현재 문자" 사이에서 발생한다. 스택은 직전 상태를 O(1)로 확인하고 제거할 수 있으므로, 연쇄 폭발(폭발 후 새로 인접한 쌍이 또 폭발)도 자연스럽게 처리된다.
- **핵심 패턴**: 연속 제거 패턴. 현재 문자를 스택 top과 비교 → 조건 충족 시 pop(폭발), 아니면 push. 연쇄 반응이 스택 구조로 자동 처리된다.
- **풀기 전 체크리스트**:
  - 추적해야 할 상태: 아직 폭발하지 않은 문자들의 순서
  - 입력: 영문자 문자열, 출력: 남은 문자열 (없으면 빈 문자열)
  - 엣지케이스: 단일 문자 (`"a"`), 폭발 없는 문자열 (`"Code"`), 연쇄 폭발 (`"xYyX"`)
- **일반적인 접근 순서**:
  1. 문자열을 순회하며 한 글자씩 처리
  2. 스택 top과 현재 문자가 같은 알파벳의 대소문자 쌍인지 확인
  3. 쌍이 맞으면 pop (폭발), 아니면 push
  4. 순회 후 스택을 join해서 반환
